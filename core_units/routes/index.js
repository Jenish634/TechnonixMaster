...
import { View, Text,Image,TouchableOpacity,SafeAreaView, ScrollView,Dimensions,CheckBox,Modal } from 'react-native';
const Routes = () => {
      ...
      await new Promise((resolve)=>{
         _.forEach(modifyRouteConfig, (routeConfig, key)=>{
            if(_.chain(routeConfig).get('dashboardWrapper', false).eq(true).value()){
               routeConfig.renderLeftButton = ()=> _.get(routeConfig,'hideLeftHeader',false)==false && ( _.get(routeConfig,'headerType')==true ? <global.components.dashboardWrapper.Header.Back/> : <global.components.dashboardWrapper.Header.Left/>)
               routeConfig.renderRightButton = ()=>_.get(routeConfig,'notification')==true ?
               <global.components.dashboardWrapper.Header.RightNotification/>:
               
                (_.get(routeConfig,'hideRightHeader',false)==false &&<global.components.dashboardWrapper.Header.Right/>);
                const Component=_.chain(routeConfig).clone().get('component').value();
               routeConfig.component = (props)=>{
                  return (_.get(routeConfig,'hideFooter',false)==false? <React.Fragment ><Component  {...props}/><global.components.dashboardWrapper.Footer/></React.Fragment>:
   ...
   const titleRender =()=>{
      return <TouchableOpacity onPress={() => global.utils.routes.navigateToAction({key: 'Dashboard', push_on_duplicate_key: false})} style={{marginTop:-12,marginLeft:-23}}><Image style={{width:150,height:70,marginLeft:-23}} source={require('../../assets/3.png')} /></TouchableOpacity>
   }
   const render=()=>{
      return (
         <React.Fragment>
