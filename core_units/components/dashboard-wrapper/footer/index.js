...
import { Ionicons, Entypo,AntDesign,FontAwesome5,MaterialIcons } from "@expo/vector-icons";
  ...
  console.log('actions');
    ...
    const renderChatMenu=()=>{
        if(!_.chain(props).get('settings.chatKittyCurrentUserData', {}).isEmpty().value()){
            return (
                <TouchableOpacity style={style.appTabContainerStyle}  onPress={() => global.utils.routes.navigateToAction({key: 'chatChannelList', push_on_duplicate_key: false})}>
    ...
    const onPressFunction =(name)=>{
      if (name =='Contacts') {
        Actions.currentScene !='Contacts' && Actions.Contacts({title:'Contacts'})  
      }else if(name=='chatsList'){
        Actions.currentScene !='chatsList' &&Actions.chatsList({title:'Messages'})
      }else if(name=='profileView'){
        Actions.currentScene !='profileView' && Actions.profileView({ title: 'Profile'})
      }else{
        global.utils.routes.navigateToAction({key: name, push_on_duplicate_key: false})
      }    }
    ...
    const render=()=>{
      ...
      return (
            <View style={style.FooterToolbar}>
                ...
                <TouchableOpacity style={style.appTabContainerStyle} 
                 onPress={() => onPressFunction('Dashboard')}
                 >
              
              <Entypo name="wallet" size={25} color={focused=='Dashboard'?"#0a356d":"#aeaeae"} />  
              {/* <Image
            ...
            /> */}
                ...
                onPress={() => onPressFunction('Contacts')} >
                  <MaterialIcons name="contacts" size={25} color={focused=='Contacts'?"#0a356d":"#aeaeae"}  />
            ...
            /> */}
                ...
                <TouchableOpacity style={style.appTabContainerStyle}  
                onPress={() =>commonAlret()} >
                <View style={{ height: 60, width: 60, backgroundColor: '#08356D', alignItems: 'center', justifyContent: 'center', borderRadius: 30, bottom: 39 }}>
              <View style={{ height: 20, width: 20, borderRadius: 5, backgroundColor: 'transparent', borderWidth: 1, alignItems: 'center', justifyContent: 'center' }}>
                <Image
                  style={{ width: 23.33, height: 21 }}
                  source={
                    focused
                ...
                <Entypo name="message" size={25} color={focused=='chatsList'?"#0a356d":"#aeaeae"}  />
                {/* <Image
              style={{ width: 23.33, height: 21 }}
              source={
                focused=='chatsList'
            ...
            /> */}
                ...
                <TouchableOpacity style={style.appTabContainerStyle} 
                onPress={() => onPressFunction('profileView')}>{console.log('user',user)}
               ...
               /> 
                :<FontAwesome5  name="user-alt" size={21} color={focused=='profileView'?"#0a356d":"#aeaeae"} />}
